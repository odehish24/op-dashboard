...
# remove outliers - replace very high values with a cap value 730
prep['count_order'] = prep['count_order'].apply(lambda x: min(x, 730))
prep = prep[:-1]

# COMMAND ----------

prep.plot(legend=True, figsize=(15,6))
...
# Fit SARIMA Model
model = SARIMAX(train['count_order'], order=(0,1,1), seasonal_order=(0, 1, 1, 8))  # Adjust p,d,q parameters as needed
model_fit = model.fit()
...
# Fit SARIMA Model
model = SARIMAX(prep['count_order'], order=(0,1,1), seasonal_order=(0, 1, 1, 8))
...
spark = SparkSession.builder.appName("freetest_colour_prediction").getOrCreate()

# Convert Pandas DataFrame to Spark DataFrame
spark_df = spark.createDataFrame(freetest_pd)

# Save as a Parquet table
spark_df.write.format("parquet").mode("overwrite").saveAsTable("freetest_colour_pred")
