...
model = Prophet(
    ...
    seasonality_prior_scale=14, #14 and 15 improve the model
...
print(f" mae: {mean_absolute_error(test['y'], forecast[-18:]['yhat'])}")
...
spark = SparkSession.builder.appName("fettle_prediction").getOrCreate()

# Convert Pandas DataFrame to Spark DataFrame
spark_df = spark.createDataFrame(fettle_pd)

# Save as a Parquet table
spark_df.write.format("parquet").mode("overwrite").saveAsTable("fettle_pred")
