...
# DBTITLE 1,Test on Train data
# Copy data to old_data and add 'isFuture' column
old_data = train_df.copy()
old_data['isFuture'] = False

# Sort old_data by index to ensure the last date is the maximum
old_data.sort_index(inplace=True)

# Get the last date from old_data
last_date = old_data.index.max()

# Calculate the future date 122 days from the last date of old_data
end_date = last_date + DateOffset(days=122)

# Create a dynamic future date range
new = pd.date_range(start=last_date + DateOffset(days=1), end=end_date, freq='D')
new_df = pd.DataFrame(index=new)
new_df['isFuture'] = True

# Merge new_df with old_data
old_new_df = pd.concat([old_data, new_df])
...
# Initialize an empty list to keep track of scores and all predictions
scores = []
predictions = []

data = old_new_df.copy()
...
for idx, (train_idx, val_idx) in enumerate(tss.split(data)):
    train1 = data.iloc[train_idx]
    test1 = data.iloc[val_idx]

    # Apply the feature aengineering
    ...
    # Append all fold's predictions to a list
    predictions.extend(y_pred)
    
    score = np.sqrt(mean_squared_error(y_pred, y_test))
    scores.append(score)

# Add only the last 122 predictions to test1 for final evaluation
test1['preds'] = predictions[-122:]
test1['actual'] = y_test

# Evaluate error
mae = round(mean_absolute_error(y_pred, y_test),2)
rmse = round(np.sqrt(mean_squared_error(y_pred, y_test)),2)
mape = round(mean_absolute_percentage_error(y_pred, y_test)*100,1)
print(f"MAE = {mae}, RMSE = {rmse}, MAPE = {mape} %")

# COMMAND ----------

ax = test1[['actual','preds']].plot(figsize=(12,4))
...
# DBTITLE 1,Evaluate error
mae = round(mean_absolute_error(y_pred2, y_test),2)
rmse = round(np.sqrt(mean_squared_error(y_pred2, y_test)),2)
mape = round(mean_absolute_percentage_error(y_pred2, y_test)*100,1)
print(f"MAE = {mae}, RMSE = {rmse}, MAPE = {mape} %")
...
# DBTITLE 1,Train final model will all data
...
spark = SparkSession.builder.appName("sh24_colour_prediction").getOrCreate()
