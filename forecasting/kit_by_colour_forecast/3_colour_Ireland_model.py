...
df.test_kit_code.value_counts().head(10)
...
prep1 = top_df.copy()
prep2 = preprocess_weekly_date(prep1)
prep = fill_missing_dates(prep2)
...
# replace high values
prep.loc[prep['y'] > 2500, 'y'] = 2500

#replace 0 values
prep.loc[prep['y'] < 100, 'y'] = 950
...
spark = SparkSession.builder.appName("ireland_colour_prediction").getOrCreate()

# Convert Pandas DataFrame to Spark DataFrame
spark_df = spark.createDataFrame(ireland_pd)

# Save as a Parquet table
spark_df.write.format("parquet").mode("overwrite").saveAsTable("ireland_colour_pred")
